...
def mail_result(user, file):
        msg = "Beste %s,\n\nOp\n  %s\nkun je je download vinden.\n\n" \
                "Karpe Noktems ledenadministratie"
        # url = reverse('ik-openvpn-download', kwargs={'file': file})
        url = 'http://karpenoktem.nl/smoelen/ik/openvpn/%s' % file
        em = EmailMessage('OpenVPN', msg % (user.first_name, url), to=
...
def create_openvpn_installer(giedo, user):
        # Remove old .exe's
        for file in glob('openvpn-install-*-%s.exe' % str(user.name)):
                os.unlink(file)

        # Set up a clean export of the openvpn-installer-repos
        dir = mkdtemp(prefix='openvpn-installer-')
        os.rmdir(dir)
        copytree(os.path.join(settings.VPN_INSTALLER_REPOS, 'installer/'), dir)
        ...
        ## Write the .nsi
        with open(os.path.join(dir, 'openvpn-gui.nsi.base'), 'r') as fh:
                nsi = fh.read()
        nsi = nsi.replace('KNUSERNAME', str(user.name))
        nsi = nsi.replace('KNOPENVPNVERSION', version)
        with open(os.path.join(dir, 'openvpn-gui.nsi'), 'w') as fh:
                fh.write(nsi)

        ## Write the OpenVPN config
        with open(os.path.join(dir, 'client.conf'), 'r') as fh:
                config = fh.read()
        config = config.replace('KNUSERNAME', str(user.name))
        with open(os.path.join(dir, 'openvpn/config', commonName + '.ovpn'),
                        'w') as fh:
                fh.write(config)

        ## Copy the keypair
        copy2(os.path.join(settings.VPN_KEYSTORE, commonName + '.key'),
                os.path.join(dir, 'openvpn/config/'))
        copy2(os.path.join(settings.VPN_KEYSTORE, commonName + '.crt'),
                os.path.join(dir, 'openvpn/config/'))

        ## Run makensis
        ph = subprocess.Popen(['makensis', 'openvpn-gui.nsi'], cwd=dir,
                        stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        ph.communicate()
        if ph.returncode != 0:
        ...
        if zip_outfile:
                ...
                ph = subprocess.Popen(['zip', os.path.join(
                        settings.VPN_INSTALLER_STORAGE, fn_zip), fn], cwd=dir,
                        stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
                ph.communicate()
                if ph.returncode != 0:
                ...
                copy2(os.path.join(dir, fn), settings.VPN_INSTALLER_STORAGE)
...
def create_openvpn_zip(giedo, user):
        # Remove old .zip's
        if os.path.exists('openvpn-config-%s.zip' % str(user.name)):
                os.unlink(file)

        # Create a temporary working directory
        dir = mkdtemp(prefix='openvpn-zip-')
        os.mkdir(os.path.join(dir, 'config'))
        ...
        with open(os.path.join(settings.VPN_INSTALLER_REPOS,
                ...
                config = fh.read()
        config = config.replace('KNUSERNAME', str(user.name))
        with open(os.path.join(dir, 'config', commonName + '.ovpn'), 'w') as fh:
                fh.write(config)
        ...
        ph = subprocess.Popen(['zip', '-r', os.path.join(
                ...
                'openvpn-config-%s.zip' % str(user.name)), 'config'], cwd=dir,
                stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        ph.communicate()
        if ph.returncode != 0:
                raise CreateInstallerException, "Zip failed"
        ...
        # Return
        mail_result(user, 'openvpn-config-%s.zip' % str(user.name))
